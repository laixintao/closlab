...
export function projectFromQuery(search: string): SavedProject | null {
  if (search.length > 8192) throw new Error('链接参数过长');
  const params = new URLSearchParams(search);
  if (![...params.keys()].some(isProjectKey)) return null;
  for (const key of params.keys()) {
    if (isProjectKey(key) && params.getAll(key).length !== 1) throw new Error(`参数 ${key} 重复`);
  }
  const number = (key: string, fallback: number): number => {
    if (!params.has(key)) return fallback;
    const value = params.get(key)!;
    if (!value.trim() || !Number.isFinite(Number(value))) throw new Error(`参数 ${key} 必须是数字`);
    return Number(value);
  };
  if (number('v', 1) !== 1) throw new Error('不支持的链接版本');
  const count = number('tiers', 2);
  if (!Number.isInteger(count) || count < 2 || count > 5) throw new Error('交换机层数必须为 2–5');
  ...
  for (const [key] of params) {
    ...
    if (!match || Number(match[1]) >= count) throw new Error(`未知交换层参数 ${key}`);
