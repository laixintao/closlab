...
import type { CapacitySummary, Filter, TopologyBuffers, TopologySpec, ViewConfig } from '../model/types';
import { useState } from 'react';

export default function Inspector({ spec, summary, graph, colorBy, selected, onSelect, filter, setFilter, path, onPath }:
  { spec: TopologySpec; summary: CapacitySummary | null; graph: TopologyBuffers | null; selected: number | null;
    ...
    path: number[]; onPath: (source: string, target: string) => void }) {
    ...
    <div className="inspector-scroll">
      {info && graph ? <section className="inspector-section node-inspector">
        ...
        <dl className="detail-list">
          ...
          <div><dt>所属平面</dt><dd>{spec.planes === 1 ? '未显式分平面' : info.plane < 0 ? '所有平面共享' : 'Plane ' + info.plane}</dd></div>
          {graph.colorGroupKind === 'connection' && <div><dt>连接分组</dt><dd>{graph.colorGroup[info.index] < 0
            ? '各组共享' : 'G' + graph.colorGroup[info.index]}</dd></div>}
        ...
        <div className="node-actions"><button onClick={() => setSource(info.label)}>设为源节点</button><button onClick={() => setTarget(info.label)}>设为目的节点</button></div>
      </section> : <>
        ...
        <section className="inspector-section">
          ...
          {summary && <dl className="detail-list">
            ...
            <div><dt>平面 / 层数</dt><dd>{spec.planes} / {spec.tiers.length}</dd></div>
          </dl>}
      ...
      <section className="inspector-section">
        ...
        <div className="filter-grid">
          ...
          <label className="field"><span>平面</span><select aria-label="筛选平面" value={filter.plane ?? 'all'} onChange={e => setFilter({ ...filter, plane: e.target.value === 'all' ? null : Number(e.target.value) })}>
            <option value="all">全部平面</option>{Array.from({ length: spec.planes }, (_, i) => <option key={i} value={i}>Plane {i}</option>)}
          </select></label>
        </div>
      ...
      <section className="inspector-section">
        ...
        <div className="path-inputs">
          <label><i className="dot teal" /><input aria-label="源节点" value={source} onChange={e => setSource(e.target.value)} placeholder="E-0" /></label>
          <div className="path-connector" />
          <label><i className="dot violet" /><input aria-label="目的节点" value={target} onChange={e => setTarget(e.target.value)} placeholder="E-1" /></label>
        </div>
        <button className="secondary-button full-width" disabled={!graph} onClick={() => onPath(source, target)}>查看最短路径<ArrowUpRight size={13} /></button>
