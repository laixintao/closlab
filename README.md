...
Each lower Pod sheet uses its Pod color for ToR and Fabric nodes, ToR–Fabric links, its boundary, and its label. The upper Fabric–Spine links, Spine nodes, and plane boundaries use the network plane color. Straight and orthogonal views follow the same rules.
